...
The main datasets that we intend on using are called Real-Time Five Minute LMPs, Instantaneous Load, and Generation by Fuel Type. The Real-Time Five Minute LMPs provide nodal pricing information with real time updates and substantial historical data. Pricing reflects usage, and can be used as a metric to assess how the correlation between weather related patterns and pricing has changed over the years, particularly at nodes associated with heavy data center usage. The Instantaneous Load is the most clear indicator of electrical usage at a given time, and also includes substantial historical data, which can be clearly used to measure changing trends between loads and their relationship to weather events. However, the Instantaneous Load is not recorded at the nodal level (just by subregion), so it is difficult to consider datacenter heavy nodes as they are diluted by the larger incorporated region, hence the need for nodal pricing. Generation by fuel type allows us to incorporate the impact of rising energy production sources that are directly tied to weather events, such as solar or wind, to account for when testing our hypothesis.
